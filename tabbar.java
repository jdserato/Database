...
public class tabbar extends AppCompatActivity {
    ...
    private BottomNavigationView.OnNavigationItemSelectedListener mOnNavigationItemSelectedListener
        ...
        public boolean onNavigationItemSelected(@NonNull MenuItem item) {
            ...
            WizardSearchFragment wizse = new WizardSearchFragment();

            switch (item.getItemId()) {
                case R.id.navigation_wizard:
                    FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                    fragmentTransaction.replace(R.id.content, wizard).commit();
                    return true;
                case R.id.navigation_house:
                    fragmentTransaction = fragmentManager.beginTransaction();
                    fragmentTransaction.replace(R.id.content, house).commit();
                    return true;
                case R.id.navigation_search:
                    fragmentTransaction = fragmentManager.beginTransaction();
                    fragmentTransaction.replace(R.id.content, search).commit();
                    return true;
                case R.id.navigation_bag:
                    fragmentTransaction = fragmentManager.beginTransaction();
                    fragmentTransaction.replace(R.id.content,wizse).commit();
                    return true;
            }
            return false;
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnLogout = (FloatingActionButton) findViewById(R.id.btnLogout); //CREATE BTNLOGOUT AGAIN LATER
